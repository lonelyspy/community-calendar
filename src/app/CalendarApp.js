...
const ADMIN_PASSWORD = "albertafilmrocks";
// ─────────────────────────────────────────────
...
const EMPTY_FORM = { name: "", date: "", place: "", time: "", description: "" };
...
export default function CalendarApp() {
  ...
  // Detect ?admin in URL to show login modal
  useEffect(() => {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    if (err) setError("Could not load events. Check your Supabase connection.");
    else setEvents(data || []);
    setLoading(false);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const { error: insertErr } = await supabase.from("events").insert([{
      ...
      place: form.place.trim(), time: form.time,
      description: form.description.trim(),
    }]);
    setSaving(false);
    if (insertErr) { setError("Could not save event. Please try again."); return; }
  ...
  const handleDelete = async (id) => {
    ...
    setDeletingId(null);
    if (delErr) setError("Could not delete event.");
  };

  // ── Save edited event ──
  const handleEditSave = async () => {
    if (!editingEvent.name.trim() || !editingEvent.date || !editingEvent.place.trim() || !editingEvent.time || !editingEvent.description.trim()) {
      setError("All fields are required."); return;
    }
    setSaving(true);
    const { error: updateErr } = await supabase.from("events").update({
      name: editingEvent.name.trim(), date: editingEvent.date,
      place: editingEvent.place.trim(), time: editingEvent.time,
      description: editingEvent.description.trim(),
    }).eq("id", editingEvent.id);
    setSaving(false);
    if (updateErr) { setError("Could not update event."); return; }
    ...
    <div style={{ minHeight: "100vh", background: "#0f0f13", fontFamily: "Georgia, serif", color: "#f0ede8", paddingBottom: 60 }}>
      ...
      {showAdminLogin && !isAdmin && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "#1a1a2e", border: "1px solid #2a2a4a", borderRadius: 16, padding: "36px 32px", width: 340, boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
            <div style={{ textAlign: "center", marginBottom: 24 }}>
              <div style={{ fontSize: 32, marginBottom: 8 }}>🔑</div>
              <h2 style={{ margin: 0, fontWeight: 400, fontSize: 20, color: "#f0ede8" }}>Admin Access</h2>
              <p style={{ margin: "6px 0 0", color: "#5e6a8a", fontSize: 13 }}>Enter your admin password to manage events</p>
            </div>
            <input
              type="password"
              ...
              autoFocus
            />
            {adminError && <p style={{ color: "#e85d4a", fontSize: 12, margin: "0 0 10px" }}>{adminError}</p>}
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button onClick={() => { setShowAdminLogin(false); setAdminPassword(""); setAdminError(""); }}
                style={{ flex: 1, padding: 11, borderRadius: 10, border: "1px solid #2a2a4a", background: "transparent", color: "#8b8baa", cursor: "pointer", fontSize: 13 }}>
                Cancel
              </button>
              <button onClick={handleAdminLogin}
                style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: "linear-gradient(135deg,#5e9aff,#3ec9a7)", color: "#0f0f13", fontWeight: 700, cursor: "pointer", fontSize: 13 }}>
                Login
              </button>
            </div>
          </div>
      ...
      {editingEvent && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ background: "#1a1a2e", border: "1px solid #2a2a4a", borderRadius: 16, padding: "32px 28px", width: "100%", maxWidth: 480, boxShadow: "0 20px 60px rgba(0,0,0,0.6)" }}>
            <h2 style={{ margin: "0 0 24px", fontWeight: 400, fontSize: 20, color: "#f0ede8" }}>✏️ Edit Event</h2>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {[
                { k: "name", l: "Event Name", t: "text" },
                { k: "date", l: "Date", t: "date" },
                { k: "place", l: "Location", t: "text" },
                { k: "time", l: "Time", t: "time" },
              ].map((f) => (
                <div key={f.k}>
                  <label style={ls}>{f.l}</label>
                  <input type={f.t} value={editingEvent[f.k]} onChange={(e) => setEditingEvent({ ...editingEvent, [f.k]: e.target.value })} style={is} />
                </div>
              ))}
              <div>
                <label style={ls}>Description</label>
                <textarea rows={3} value={editingEvent.description} onChange={(e) => setEditingEvent({ ...editingEvent, description: e.target.value })} style={{ ...is, resize: "vertical" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
              ...
              <button onClick={handleEditSave} disabled={saving}
                style={{ flex: 2, padding: 11, borderRadius: 10, border: "none", background: saving ? "#2a2a4a" : "linear-gradient(135deg,#5e9aff,#3ec9a7)", color: saving ? "#8b8baa" : "#0f0f13", fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", fontSize: 13 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 8, flexWrap: "wrap" }}>
          {["calendar", "form"].map((v) => (
            <button key={v} onClick={() => { setView(v); setSelDay(null); }}
              style={{ padding: "8px 20px", borderRadius: 24, border: "1px solid", borderColor: view === v ? "#5e9aff" : "#3a3a5c", background: view === v ? "rgba(94,154,255,0.15)" : "transparent", color: view === v ? "#5e9aff" : "#8b8baa", cursor: "pointer", fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              {v === "calendar" ? "📆 View Calendar" : "✏️ Add Event"}
            </button>
          ))}
          {isAdmin ? (
            <button onClick={() => setIsAdmin(false)}
              style={{ padding: "8px 16px", borderRadius: 24, border: "1px solid #f4a228", background: "rgba(244,162,40,0.15)", color: "#f4a228", cursor: "pointer", fontSize: 12, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              🔑 Admin — Exit
            </button>
          ) : null}
        </div>
        {isAdmin && (
          <p style={{ margin: "10px 0 0", color: "#f4a228", fontSize: 11, letterSpacing: "0.08em" }}>
            ⚡ ADMIN MODE ACTIVE — edit and delete buttons are now visible
          </p>
        )}
      </div>
      ...
      {error && (
        <div style={{ background: "rgba(232,93,74,0.12)", border: "1px solid #e85d4a", borderRadius: 10, padding: "12px 20px", margin: "20px auto", maxWidth: 600, color: "#e85d4a", fontSize: 13, textAlign: "center" }}>
          ⚠️ {error}&nbsp;&nbsp;
          <span style={{ cursor: "pointer", textDecoration: "underline" }} onClick={() => setError(null)}>Dismiss</span>
        </div>
      )}
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "0 14px" }}>
          ...
          <div style={{ paddingTop: 28 }}>
            ...
            <div style={{ background: "#1a1a2e", borderRadius: 14, border: "1px solid #2a2a4a", overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.5)" }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", background: "#12122a" }}>
                {DAYS.map((d) => <div key={d} style={{ padding: "12px 4px", textAlign: "center", fontSize: 11, letterSpacing: "0.08em", color: "#5e6a8a" }}>{d}</div>)}
              </div>
                    ...
                    {isAdmin && (
                      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                        <button onClick={() => setEditingEvent({ ...ev })}
                          style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid #3a3a6a", background: "rgba(94,154,255,0.1)", color: "#5e9aff", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                          ✏️ Edit
                        </button>
                        <button onClick={() => handleDelete(ev.id)} disabled={deletingId === ev.id}
                          style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid #4a2a2a", background: "rgba(232,93,74,0.1)", color: "#e85d4a", cursor: deletingId === ev.id ? "not-allowed" : "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
            ...
            {!loading && monthEvents.length === 0 && !error && (
              <div style={{ textAlign: "center", marginTop: 48, color: "#5e6a8a" }}>
                ...
                <p style={{ fontSize: 14 }}>No events in {MONTHS[selMonth]}.<br />
                  <span style={{ color: "#5e9aff", cursor: "pointer", textDecoration: "underline" }} onClick={() => setView("form")}>Add the first one →</span>
                </p>
              </div>
            )}
          </div>
        ...
        {view === "day" && selDay !== null && (
          <div style={{ paddingTop: 28 }}>
            <button onClick={() => { setView("calendar"); setSelDay(null); }}
              style={{ background: "none", border: "none", color: "#5e9aff", cursor: "pointer", fontSize: 14, marginBottom: 18, display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                  ...
                  {isAdmin && (
                    <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                      <button onClick={() => setEditingEvent({ ...ev })}
                        style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid #3a3a6a", background: "rgba(94,154,255,0.1)", color: "#5e9aff", cursor: "pointer", fontSize: 12 }}>
                        ✏️ Edit
                      </button>
                      <button onClick={() => handleDelete(ev.id)} disabled={deletingId === ev.id}
                        style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid #4a2a2a", background: "rgba(232,93,74,0.1)", color: "#e85d4a", cursor: deletingId === ev.id ? "not-allowed" : "pointer", fontSize: 12 }}>
                        {deletingId === ev.id ? "…" : "🗑️ Delete"}
                      </button>
                    </div>
                  )}
        ...
        {view === "form" && (
          <div style={{ paddingTop: 36, maxWidth: 520, margin: "0 auto" }}>
            <h2 style={{ textAlign: "center", fontWeight: 400, fontSize: 24, color: "#f0ede8", marginBottom: 6 }}>Submit an Event</h2>
            <p style={{ textAlign: "center", color: "#5e6a8a", fontSize: 13, marginBottom: 28 }}>Fill out the form below to add your event to the community calendar.</p>
            {submitted && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <button onClick={handleSubmit} disabled={saving}
                style={{ marginTop: 6, padding: "14px 0", borderRadius: 12, background: saving ? "#2a2a4a" : "linear-gradient(135deg,#5e9aff,#3ec9a7)", border: "none", color: saving ? "#8b8baa" : "#0f0f13", fontSize: 15, fontWeight: 700, letterSpacing: "0.05em", cursor: saving ? "not-allowed" : "pointer", boxShadow: saving ? "none" : "0 4px 20px rgba(94,154,255,0.3)" }}>
